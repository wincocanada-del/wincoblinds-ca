import {protectedBlock,pageAddressError,previewSlug} from './editor-helpers.mjs';
...
let user=null,role=null,data=null,version=0,dirty=false,tab='dashboard',selected='index',busy=false,history=[],members=[],mediaTarget=null,mediaUrls={},publication=null,poll=null,pendingRelease=null,uploading=false,validationErrors=[];
...
function mark(){dirty=true;validationErrors=[];document.querySelector('#validation-errors')?.remove();document.querySelector('#save-state')?.replaceChildren(document.createTextNode('저장하지 않은 변경'));}
...
function imageField(label,value,scope,attrs=''){return `<div class="image-label"><span>${esc(label)}</span><div class="image-field"><img src="${esc(mediaUrls[value]||('/'+value.replace(/^\//,'')))}" alt="선택한 사진"><div>${button('사진 선택 / 변경','choose-image',`data-scope="${scope}" ${attrs}`)}<small>${esc(value)}</small></div></div></div>`;}
...
async function load(){
 ...
 try{const state=await rpc('state');role=state.role;data=state.draft?.data||clone(seed);version=state.draft?.version||0;dirty=false;publication=await liveVersion();await refreshMedia();render();}catch(error){if(error.message==='Not authorized'){login('로그인은 확인됐지만 홈페이지 관리 권한이 없습니다. 관리 담당자에게 이메일 등록을 요청해 주세요.');root.insertAdjacentHTML('beforeend','<div style="text-align:center">'+button('로그아웃','logout')+'</div>');}else{root.innerHTML='<section class="login"><h1>관리 화면 연결을 확인해 주세요</h1><p>내용을 불러오지 못했습니다. 인터넷 연결을 확인한 뒤 다시 시도해 주세요.</p>'+button('다시 불러오기','retry-load')+'</section>';}}
}
...
function render(){
 for(const el of root.querySelectorAll('details[data-detail]'))detailState.set(el.dataset.detail,el.open);
 ...
 root.innerHTML=`<header class="app-header"><div><img src="/assets/images/logo/winco-original.png" alt="Winco"><div class="identity">${esc(user.email)} · ${esc(role)}</div></div><div class="actions"><span id="save-state" class="status-pill">${demo?'로컬 테스트 · 실제 저장 안 됨':dirty?'저장하지 않은 변경':`저장 버전 ${version}`}</span>${button('초안 저장','save','', 'primary')}${button('미리보기','preview')}${['owner','publisher'].includes(role)?button(pendingRelease?'게시 확인 중…':'게시','publish',pendingRelease?'disabled':'','primary'):''}<a href="/" target="_blank" rel="noopener">홈페이지 ↗</a>${button('로그아웃','logout')}</div></header><div class="layout"><aside class="sidebar">${nav.map(([key,label])=>button(label,'tab',`data-tab="${key}"${tab===key?' aria-current="page"':''}`)).join('')}<div class="foot">문의 알림은 Winco Gmail로 전달됩니다.<br><a href="https://app.netlify.com/projects/wincohomepage/forms" target="_blank" rel="noopener">문의함 열기 ↗</a></div></aside><main>${validationErrors.length?`<section id="validation-errors" class="error-list" role="alert" tabindex="-1"><h2>아래 내용을 확인해 주세요</h2><ul>${validationErrors.map(e=>`<li>${esc(e)}</li>`).join('')}</ul><p>내용을 수정한 뒤 저장 또는 미리보기를 다시 눌러주세요.</p></section>`:''}${content()}</main></div>`;
 for(const el of root.querySelectorAll('details[data-detail]'))if(detailState.has(el.dataset.detail))el.open=detailState.get(el.dataset.detail);
 const links=document.querySelector('#page-addresses');links.innerHTML=data.pages.filter(p=>p.visible).map(p=>`<option value="${p.slug==='index'?'/':'/'+esc(p.slug)}">${esc(p.title)}</option>`).join('');
}
function content(){
...
function fieldEditor(b,f,i){const value=b.values[f.key]||'';const attrs=`data-block="${i}" data-key="${f.key}"`;if(f.kind==='image')return imageField(f.label,value,'block',attrs);if(f.kind==='text')return `<label><span>${esc(f.label)}</span><textarea ${attrs}>${esc(value)}</textarea></label>`;return input(f.label,value,attrs+(f.kind==='url'?' list="page-addresses"':''));}
function valid(){validationErrors=validateSite(data);if(!validationErrors.length)return true;render();document.querySelector('#validation-errors').focus();return false;}
async function save(){
...
async function showMedia(){
 ...
 if(!demo)await Promise.all([...items.keys()].filter(x=>x.startsWith('/media/')).map(async path=>{const r=await api.storage.from('winco-website').createSignedUrl(path.slice(7),3600);if(r.data)mediaUrls[path]=r.data.signedUrl;}));
 document.querySelector('#media-search').value='';
 document.querySelector('#media-grid').innerHTML=[...items].map(([src])=>{const name=data.media.find(m=>'/media/'+m.path===src)?.name||src.split('/').pop();return `<button data-name="${esc(name.toLowerCase())}" data-action="pick-image" data-src="${esc(src)}"><img loading="lazy" src="${esc(mediaUrls[src]||'/'+src.replace(/^\//,''))}" alt="사진 미리보기">${esc(name)}</button>`;}).join('');
 document.querySelector('#media-count').textContent=items.size+'장';
 if(!document.querySelector('#media-dialog').open)document.querySelector('#media-dialog').showModal();
}
function ask(message){
 const dialog=document.querySelector('#decision-dialog');dialog.querySelector('p').textContent=message;dialog.returnValue='';dialog.showModal();
 return new Promise(resolve=>dialog.addEventListener('close',()=>resolve(dialog.returnValue==='confirm'),{once:true}));
}
async function drawPreview(slug){
 await refreshMedia();const p=data.pages.find(p=>p.slug===slug)||current();
 document.querySelector('#preview-page').value=p.slug;
 document.querySelector('#preview-dialog iframe').srcdoc=renderPage(data,p,{preview:true,mediaUrls});
}
...
async function action(el){
 ...
 if(a==='retry-load'){await load();return;}
 if(a==='logout'){if(dirty&&!await ask('저장하지 않은 변경이 있습니다. 로그아웃할까요?'))return;await api.auth.signOut({scope:'local'});location.href='/admin/';return;}
 ...
 if(a==='preview'){
  if(!valid())return;
  document.querySelector('#preview-page').innerHTML=data.pages.map(p=>`<option value="${esc(p.slug)}">${esc(p.title)}${p.visible?'':' (숨김 초안)'}</option>`).join('');
  await drawPreview(previewSlug(tab,selected));document.querySelector('#preview-dialog').showModal();return;
 }
 ...
 if(a==='close-page'){document.querySelector('#page-dialog').close();return;}
 ...
 if(a==='publish'){if(pendingRelease){toast('앞선 게시의 반영을 확인하고 있습니다.');return;}document.querySelector('#publish-dialog').showModal();return;}
 if(a==='confirm-publish'){
  ...
  const{data:session}=await api.auth.getSession();if(!session.session?.access_token)throw Error('로그인이 만료됐습니다. 현재 창은 유지하고 다른 창에서 관리자 로그인 후 다시 시도해 주세요.');
  ...
  pendingRelease=result.release;toast('게시를 시작했습니다. 실제 반영 여부를 확인하고 있습니다.');tab='dashboard';render();document.querySelector('#publish-status').textContent='게시 준비 중 · 잠시 후 실제 홈페이지 반영을 확인합니다.';
  clearInterval(poll);let count=0;poll=setInterval(async()=>{const live=await liveVersion();count++;if(live?.release===result.release){clearInterval(poll);pendingRelease=null;publication=live;render();document.querySelector('#publish-status')?.replaceChildren(document.createTextNode('게시 완료 · '+date(live.builtAt)));toast('홈페이지 게시가 완료됐습니다.');}else if(count>=60){clearInterval(poll);pendingRelease=null;render();document.querySelector('#publish-status')?.replaceChildren(document.createTextNode('게시 완료를 아직 확인하지 못했습니다. Netlify 배포 상태를 확인한 뒤 다시 게시할 수 있습니다.'));}},6000);return;
 }
 if(a==='restore'){if(!await ask('선택한 버전의 내용을 편집 화면으로 불러올까요? 현재 저장하지 않은 변경은 대체됩니다.'))return;const old=await rpc('version',{p_id:el.dataset.id});data=old.data;dirty=true;await refreshMedia();tab='pages';selected='index';render();toast('이전 내용을 불러왔습니다. 초안 저장 후 미리보기와 게시를 진행하세요.');return;}
 if(a==='member-toggle'){if(!await ask(el.dataset.email+'의 접근 권한을 변경할까요?'))return;await rpc('member_set',{p_email:el.dataset.email,p_role:el.dataset.role,p_active:el.dataset.active==='true'});members=await rpc('member_list');render();return;}
 if(a==='add-page'){
  const dialog=document.querySelector('#page-dialog');dialog.querySelector('form').reset();dialog.querySelector('[name="kind"]').value=el.dataset.kind;dialog.querySelector('#page-error').textContent='';dialog.showModal();return;
 ...
 else if(a.startsWith('block-')){const list=current().blocks,b=list[i];if(protectedBlock(b)&&['block-toggle','block-remove'].includes(a)){toast('큰 제목과 고객 문의의 기본 구성은 유지됩니다. 내용과 사진을 수정해 주세요.');return;}if(a==='block-up')move(list,i,-1);if(a==='block-down')move(list,i,1);if(a==='block-toggle')b.hidden=!b.hidden;if(a==='block-remove'){if(!await ask('이 내용 블록을 초안에서 삭제할까요?'))return;list.splice(i,1);}}
 ...
 else if(a==='menu-remove'){if(!await ask('메뉴를 초안에서 삭제할까요? 연결된 페이지는 유지됩니다.'))return;data.navigation.splice(i,1);}
 ...
 else if(a==='gallery-remove'){if(!await ask('갤러리에서 이 항목을 삭제할까요? 원본 사진과 과거 기록은 유지됩니다.'))return;data.gallery.splice(i,1);}
 ...
 else if(a==='fabric-remove'){if(!await ask('원단 항목을 초안에서 삭제할까요?'))return;data.catalog[group].splice(i,1);}
 else return;
 mark();render();
}
document.addEventListener('click',async event=>{const el=event.target.closest('[data-action]');if(!el||busy)return;event.preventDefault();busy=true;root.inert=true;el.disabled=true;try{await action(el);}catch(error){toast(error.message||'처리하지 못했습니다. 다시 시도해 주세요.');}finally{busy=false;root.inert=false;el.disabled=false;if(validationErrors.length)document.querySelector('#validation-errors')?.focus();}});
document.addEventListener('input',event=>{const el=event.target,d=el.dataset;if(el.id==='media-search'){let count=0;for(const b of document.querySelectorAll('#media-grid button')){b.hidden=!b.dataset.name.includes(el.value.trim().toLowerCase());if(!b.hidden)count++;}document.querySelector('#media-count').textContent=count+'장';return;}if(!data)return;
...
document.addEventListener('submit',async event=>{if(event.target.method==='dialog')return;event.preventDefault();if(busy)return;busy=true;root.inert=true;const form=event.target,submit=form.querySelector('button[type=submit]');if(submit)submit.disabled=true;
 try{const values=new FormData(form);
 if(form.id==='page-form'){
  const slug=String(values.get('slug')).trim(),title=String(values.get('title')).trim(),kind=String(values.get('kind'));
  const error=pageAddressError(slug,data.pages);if(error){document.querySelector('#page-error').textContent=error;return;}
  data.pages.push({slug,title,description:'페이지 소개를 입력해 주세요.',kind,visible:false,blocks:[{id:crypto.randomUUID(),template:'intro',hidden:false,values:{eyebrow:'WINCO',title,body:'내용을 입력해 주세요.'}}]});selected=slug;tab=kind==='post'?'posts':'pages';document.querySelector('#page-dialog').close();mark();render();toast('숨김 초안을 만들었습니다. 내용을 작성한 뒤 게시에 포함하고 메뉴를 연결해 주세요.');return;
 }
 if(form.id==='login-form'){const email=String(values.get('email')).trim().toLowerCase();const{error}=await api.auth.signInWithOtp({email,options:{emailRedirectTo:location.origin+'/admin/',shouldCreateUser:true}});if(error)throw error;toast('로그인 메일을 보냈습니다. 받은편지함과 스팸함을 확인해 주세요.');}
 ...
 catch(error){toast(error.message);}finally{busy=false;root.inert=false;if(submit)submit.disabled=false;}
});
document.querySelector('#upload-file').addEventListener('change',async event=>{
 const file=event.target.files[0];if(!file||busy)return;if(demo){toast('사진 업로드는 실제 관리자 로그인 후 사용할 수 있습니다.');return;}
 busy=true;uploading=true;root.inert=true;document.querySelector('#media-dialog').setAttribute('aria-busy','true');for(const el of document.querySelectorAll('#media-dialog button,#media-dialog input'))el.disabled=true;document.querySelector('#upload-progress').textContent='사진을 업로드하고 있습니다…';
 try{
 ...
 }catch(error){toast(error.message);}finally{event.target.value='';busy=false;uploading=false;root.inert=false;document.querySelector('#media-dialog').removeAttribute('aria-busy');for(const el of document.querySelectorAll('#media-dialog button,#media-dialog input'))el.disabled=false;document.querySelector('#upload-progress').textContent='';}
});
document.querySelector('#media-dialog').addEventListener('cancel',event=>{if(uploading)event.preventDefault();});
document.querySelector('#preview-page').addEventListener('change',async event=>{try{await drawPreview(event.target.value);}catch(error){toast(error.message);}});
addEventListener('beforeunload',event=>{if(dirty||uploading){event.preventDefault();event.returnValue='';}});
load();
